...
class GRU(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(         # if use nn.RNN(), it hardly learns
            ...
            hidden_size=64,         # gru hidden unit
            num_layers=2,           # number of gru layer
    ...
    def forward(self, x):
        ...
        r_out, (h_n, h_c) = self.gru(x, None)   # None represents zero initial hidden state
...
for epoch in range(EPOCH):
    for step, (x, y) in enumerate(train_loader):   # gives batch data, normalize x when iterate train_loader
        ...
        # output, r_output, h_net, h_cet = gru(b_x)               # gru output
        # print(output)
        # input()
        # print(r_output)
        # input()
